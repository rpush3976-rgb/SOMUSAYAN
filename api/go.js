...
// 🔥 HARDCODED KEYS (Temporary - Baad mein hata dena)
process.env.TURNSTILE_SITE_KEY = '0x4AAAAAAEZYYn1-vYD-WxW';
process.env.TURNSTILE_SECRET_KEY = '0x4AAAAAAEZYYLwth6wkYRnr';
process.env.JWT_SECRET = 'Somu@2026#StrongKey';
process.env.NEXT_PUBLIC_APP_URL = 'https://somusayan.vercel.app';
        ...
        if (req.method === 'GET') {
            if (reveal && t) {
                try {
                    ...
                    if (now < decoded.exp - 30) return res.status(403).json({ error: 'Wait' });
                    return res.status(200).json({ url: decoded.url });
                } catch (e) { return res.status(400).json({ error: 'Invalid' }); }
            }
            ...
            const token = jwt.sign({ url, exp: Math.floor(Date.now()/1000) + 120 }, process.env.JWT_SECRET);
